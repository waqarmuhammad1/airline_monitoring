#!/usr/bin/env node
/**
 * Playwright United Airlines Automation — Standalone Single File (Stealth, JS)
 *
 * Install & run:
 *   npm init -y
 *   npm install playwright-extra puppeteer-extra-plugin-stealth
 *   npx playwright install chromium
 *   node united-automation.js
 */
...
function log(level, ctx, msg, err) {
...
function saveScreenshot(filePath, buffer) {
  ensureDir(path.dirname(filePath));
...
async function withRetry(fn, label = "operation", maxRetries = MAX_RETRIES, backoffMs = RETRY_BACKOFF_MS) {
  let lastError;
...
async function checkGoogleLogin(page) {
  try {
...
async function navigateToUnited(page) {
...
async function findGridElements(page) {
...
async function traverseGrid(page) {
  ...
  for (let i = 0; i < total; i++) {
    ...
    try {
      ...
      record.success = true;
      log("INFO", "grid", `Clicked ${i}: "${String(record.textContent).substring(0, 60)}"`);
